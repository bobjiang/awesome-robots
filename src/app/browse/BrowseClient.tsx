...
function BrowseContent({ robots, categories }: BrowseClientProps) {
  ...
  const filteredRobots = useMemo(() => {
    const filtered = robots.filter((robot) => {
      ...
      if (searchQuery) {
        ...
        return (
          ...
          (robot.description && robot.description.toLowerCase().includes(query)) ||
    ...
    return filtered;
  }, [robots, filters, searchQuery, sortBy]);

  const availableBrands = Array.from(new Set(robots.map((robot) => robot.brand)));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8 flex flex-col lg:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <label htmlFor="browse-sort" className="text-sm font-medium text-gray-700">Sort by:</label>
            <select
              id="browse-sort"
              value={sortBy}
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredRobots.map((robot) => (
                    <ProductCard key={robot.id} robot={robot as never} />
                  ))}
                </div>
              </>
            ) : (
...
export default function BrowseClient({ robots, categories }: BrowseClientProps) {
  return (
    <Suspense fallback={
      <Layout>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <BrowseContent robots={robots} categories={categories} />
    </Suspense>
  );
}
